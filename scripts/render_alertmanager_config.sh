...
# Silence-burn folder route (defaults to judge Slack webhook)
export RAG_SILENCE_BURN_SLACK_WEBHOOK="${RAG_SILENCE_BURN_SLACK_WEBHOOK:-$RAG_ALERTMANAGER_SLACK_WEBHOOK}"

if command -v envsubst >/dev/null 2>&1; then
  envsubst '${RAG_ALERTMANAGER_SLACK_WEBHOOK} ${RAG_ALERTMANAGER_WEBHOOK_URL} ${RAG_ALERTMANAGER_INGEST_WEBHOOK_URL} ${RAG_ALERTMANAGER_INGEST_DLQ_QUARANTINE_WEBHOOK_URL} ${RAG_SILENCE_BURN_SLACK_WEBHOOK}' \
    < "$TEMPLATE" > "$OUTPUT"
else
  ...
  sed \
    ...
    -e "s|\${RAG_SILENCE_BURN_SLACK_WEBHOOK}|${RAG_SILENCE_BURN_SLACK_WEBHOOK}|g" \
    "$TEMPLATE" > "$OUTPUT"
fi
