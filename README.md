...
Mute snapshot export: `python -m rag.cli judge-ack-digest --export-mute-snapshots --export-format csv` (mute store + keep-on-mute digest snapshot). Slack files.upload: `--upload-mute-snapshots` veya Block Kit **Revoke export** confirm modal (`judge_ack_digest_revoke_mute_export` → `judge_mute_export_revoke_modal`) / **Mute snapshots** (`judge_ack_digest_export_mute_snapshots`). Schedule CI: `.github/workflows/judge-ack-digest.yml` → CSV artifact + opsiyonel Slack upload (`RAG_JUDGE_SLACK_BOT_TOKEN`, kanal: `RAG_JUDGE_ACK_DIGEST_MUTE_UPLOAD_CHANNEL`). Retention: `--prune-mute-exports` / `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_RETENTION_DAYS` (`metadata/mute_exports/`). Signed URL: `--sign-mute-export` → `GET /judge/mute-snapshots?file=&expires=&jti=&sig=` (`RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_SIGNING_SECRET`; store: `mute_export_signed_urls.json`). Revoke + audit: `--revoke-mute-export JTI|FILE` / admin UI `GET|POST /judge/mute-export-revoke` / Block Kit confirm modal + audit Slack thread reply + rate-limit (`RAG_JUDGE_ACK_RATE_LIMIT_SEC`) + digest-diff annotate (`mute_export_revokes`; `mute_export_sign` / `mute_export_revoke`; `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_CONFIRM=0` ile anında revoke; thread: `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_THREAD`). Canvas refresh on revoke: `chat.update` digest actions/context + mute fan-out sync (`RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_CANVAS`; button value `tenant|latest`). Fan-out audit trail: `mute_export_revoke_fanout` JSONL + digest-diff `mute_export_revoke_fanouts` + Slack thread (`RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_FANOUT_AUDIT` / `_FANOUT_THREAD`). Grafana annotate: `post_grafana_annotation` → `/api/annotations` (`RAG_GRAFANA_URL` / `RAG_GRAFANA_API_KEY`, tags `mute-export-revoke-fanout`; dashboard `rag-judge-soft-fail`; panel id 27 `annolist` “Mute export revoke fan-out annotations”; `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_FANOUT_GRAFANA` / `_GRAFANA_PANEL`). Annotation link: `grafana_annotation_deep_link` (`/d/{uid}/{slug}?orgId=1&viewPanel=27`, `RAG_GRAFANA_PUBLIC_URL` / `_GRAFANA_LINK`; Slack thread `grafana=`). Explore: `grafana_annotation_explore_link` (`/explore?orgId=1&left=` annotations query, tags `mute-export-revoke-fanout`; `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_FANOUT_GRAFANA_EXPLORE`; Slack `explore=`). Query: `query_grafana_annotations` GET `/api/annotations` (`RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_FANOUT_GRAFANA_QUERY` / `_QUERY_LIMIT`; Slack `annotations=`). Prune: `prune_grafana_annotations` DELETE `/api/annotations/:id` (`RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_FANOUT_GRAFANA_PRUNE` / `_PRUNE_DAYS` / `--prune-grafana-annotations`; Slack `pruned=`). Fan-out audit retention: `--prune-mute-export-revoke-fanouts` / `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_REVOKE_FANOUT_RETENTION_DAYS` / `_KEEP` / `_PRUNE` (yalnız `mute_export_revoke_fanout` satırları). TTL sweep: `--sweep-mute-export-urls` / `RAG_JUDGE_ACK_DIGEST_MUTE_EXPORT_SWEEP=1` (`mute_export_sweep`).
Signing sidecar forward metrik: `rag_webhook_signing_sidecar_forward_total{mode,result}`. Grafana alerts: `grafana/rules/rag_webhook_signing_sidecar.yml` + `grafana/alerting/rag_webhook_signing_sidecar.yaml` (`RagWebhookSigningSidecarForwardFail` / burst; DLQ quarantine HMAC yolu). mTLS cert expiry: `rag_webhook_signing_sidecar_cert_expiry_days{role}` + `RagWebhookSigningSidecarCertExpirySoon` / `--check-certs`. Auto-rotate hook: `--rotate-certs` / `--rotate-certs-if-expiring` (`metadata/sidecar-tls` + archive). Schedule CI: `.github/workflows/webhook-signing-sidecar-rotate.yml` (günlük + `workflow_dispatch`). Rotate notify: `--notify` / `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_SLACK_WEBHOOK`; Block Kit digest (per-role days_left): `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_DIGEST=1` (default on). Digest thread: `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD` + `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_TS` / `_CHANNEL` (`resolve_sidecar_rotate_notify_channel`: env → persisted `channel_id` → DLQ/judge channel; webhook `channel` + digest context) / persist `parent_ts` + history (`RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_PERSIST` / `_KEEP`, state `metadata/sidecar_rotate_notify_thread.json`). Thread reply broadcast: `broadcast_sidecar_rotate_notify_thread_reply` / `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_BROADCAST` + `_BROADCAST_CHANNELS` (JSON veya virgüllü kanal listesi; primary channel atlanır). Thread reply ack: `ack_sidecar_rotate_notify_thread_reply` / `reactions.add` (`RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_ACK` / `_ACK_EMOJI`, default `white_check_mark`; persist `last_ack_ts`). Ack persist: `persist_sidecar_rotate_notify_thread_ack` / `ack_history` (`RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_ACK_PERSIST` / `_ACK_KEEP`; skip `already_acked_persisted`). Ack history: `list_sidecar_rotate_notify_thread_ack_history` / `format_sidecar_rotate_notify_thread_ack_history` (`acks=N last_ack=`; `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_ACK_HISTORY` / `_ACK_HISTORY_LIMIT`) / `prune_sidecar_rotate_notify_thread_ack_history` (`--prune-rotate-ack-history` / `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_NOTIFY_THREAD_ACK_PRUNE` / `_ACK_TTL_DAYS`; auto on rotate notify). Fail → PagerDuty (`RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_PAGERDUTY_ROUTING_KEY` / `notify_pd`; severity: `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_PD_SEVERITY` veya `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_PD_SEVERITY_BY_ERROR` JSON). Dry-run CI gate: `--dry-run` + `RAG_WEBHOOK_SIGNING_SIDECAR_ROTATE_DRY_RUN_GATE=1` → exit 2 when `would_rotate` (workflow_dispatch `dry_run=true`).
Inhibit equal canary resolve alert: `grafana/rules/rag_inhibit_equal.yml` + `grafana/alerting/rag_inhibit_equal_canary.yaml` (`RagInhibitEqualCanaryResolveFail`). Green apply sonrası auto-silence: `INHIBIT_EQUAL_CANARY_AUTO_SILENCE=1` (`_DURATION=2h`, metrik: `rag_inhibit_equal_canary_silence_total` + Grafana panelleri). Silence burn-rate: `RagInhibitEqualCanarySilenceBurn` (`rag:inhibit_equal_canary_silence_fail_ratio:1h/6h`; Grafana folder `RAG Silence Burn` + label `grafana_folder=rag-silence-burn`; Alertmanager exclusive folder receiver policy (`silence-burn-webhook` / `silence-burn-slack`, `group_wait=15s`, `group_interval=2m`, `repeat_interval=1h`, `continue: false`, mute time intervals `silence-burn-quiet` 22:00–08:00 UTC + weekend, active time intervals `silence-burn-business` weekday 08:00–22:00 UTC, mute-window inhibit: source `RagInhibitEqualCanarySilenceBurn` → target `grafana_folder=rag-silence-burn|RAG Silence Burn` equal `grafana_folder`+service+team; critical→warning equal grafana_folder for `grafana_folder=rag-silence-burn` and `grafana_folder="RAG Silence Burn"`; critical→warning equal alertname+service+team (equal team) when `grafana_folder=~rag-silence-burn|RAG Silence Burn`) + Grafana contact `rag-silence-burn-slack`; runbook: `/ops/silence-burn`, amtool: `/ops/amtool`; Slack canary button + PagerDuty/Opsgenie `details.runbook_url` via `INHIBIT_EQUAL_CANARY_PD_RUNBOOK_URL`; Opsgenie tags: `silence-burn`, `runbook`, `runbook:silence-burn`, `region:*`, opsiyonel `INHIBIT_EQUAL_CANARY_OPSGENIE_TAGS`; Grafana annotation + `details.opsgenie_url` deep-link via `opsgenie_alert_deep_link` / `INHIBIT_EQUAL_CANARY_OPSGENIE_ALERT_URL`; green recover close+ack multi-region sync via `INHIBIT_EQUAL_CANARY_CLOSE_ACK_SYNC` / `opsgenie_ack_regions`). Silence expiry webhook: `alertmanager --canary-silence-expiry` / `.github/workflows/inhibit-equal-canary-silence-expiry.yml` (`INHIBIT_EQUAL_CANARY_SILENCE_EXPIRY_WEBHOOK`).
...
- Judge: mute export revoke fan-out Grafana annotation patch
- Ingest: dual-write quarantine sidecar rotate notify digest thread reply ack TTL sweep
- Observability: silence burn folder inhibition equal severity
