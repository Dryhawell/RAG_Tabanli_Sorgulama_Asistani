...
Mute snapshot export: `python -m rag.cli judge-ack-digest --export-mute-snapshots --export-format csv` (mute store + keep-on-mute digest snapshot). Slack files.upload: `--upload-mute-snapshots` veya Block Kit **Mute snapshots** (`judge_ack_digest_export_mute_snapshots`).
Signing sidecar forward metrik: `rag_webhook_signing_sidecar_forward_total{mode,result}`.
...
- Judge: digest mute snapshot export schedule CI
- Ingest: dual-write DLQ quarantine sidecar Grafana alerts
- Observability: inhibit equal canary silence burn-rate rule
