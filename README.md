...
Slack ack deep-link: `RAG_JUDGE_ACK_PUBLIC_URL` → `/judge/ack-form`; interactive: `RAG_JUDGE_SLACK_SIGNING_SECRET` + `POST /judge/slack-interactive` (modal: `RAG_JUDGE_SLACK_BOT_TOKEN`; ack thread reply + `chat.postEphemeral`; rate limit: `RAG_JUDGE_ACK_RATE_LIMIT_SEC`; kanal yoksa `conversations.history`; parent: `conversations.replies`).
Observability stack: `docker compose --profile obs up -d` (Prometheus + Alertmanager + Tempo + Grafana).
Alertmanager Slack: `RAG_ALERTMANAGER_SLACK_WEBHOOK` (+ opsiyonel `RAG_ALERTMANAGER_WEBHOOK_URL`) → `scripts/render_alertmanager_config.sh`; rotate/reload: `python -m rag.cli alertmanager --rotate-slack-webhook URL`; silence: `python -m rag.cli alertmanager --silence --matcher 'service=rag-judge' --duration 2h`.
...
Dual-write lag / catch-up / cutover: `python -m rag.cli migrate-vector --lag-report` · `--catch-up` · `--catch-up --auto-cutover` · `--cutover --catch-up --target qdrant` (cron: `.github/workflows/dual-write-catch-up.yml`).
...
- Judge: Slack ack audit trail export (CSV/JSONL)
- Ingest: dual-write shadow-read compare (primary vs secondary search)
- Observability: Alertmanager inhibit rule generator from Grafana labels
